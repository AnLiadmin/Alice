...
<script type="text/javascript">
	function pNum(cid,p,no){  //点击+数量
		var nums = parseInt($("#num_count"+no).val())+1;
		location.href = "cart?action=update&cid="+cid+"&num="+nums+"&price="+p
	}
	function mNum(cid,p,no){ //点击-数量
		var nums = parseInt($("#num_count"+no).val())-1;
		if(nums>0){
			location.href = "cart?action=update&cid="+cid+"&num="+nums+"&price="+p
		}else{
			if(confirm("确认要删除吗?")){
				location.href="cart?action=deleteCart&cid="+cid;
			}
		}
	}
